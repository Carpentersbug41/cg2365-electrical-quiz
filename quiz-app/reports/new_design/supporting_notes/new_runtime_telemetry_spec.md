...
## 18. Additional Measurement Categories

These categories should also be measured because they affect both instructional quality and product optimization.

### 18.1 Support dependence
Measure whether the learner is learning independently or being carried by scaffolds.

Recommended fields:
- `hint_count_per_chunk`
- `hint_count_before_success`
- `repair_count_before_success`
- `supported_success_rate`
- `unsupported_success_rate`
- `support_fade_success_rate`

Key question:
- Is the learner becoming less dependent on support over time, or only succeeding when heavily scaffolded?

### 18.2 Transfer quality
Measure whether knowledge can be used in new contexts, not just recalled.

Recommended fields:
- `near_transfer_question_shown`
- `near_transfer_correctness`
- `far_transfer_question_shown` nullable
- `far_transfer_correctness` nullable
- `transfer_success_rate`
- `transfer_after_explanation_rate`
- `transfer_after_repair_rate`

Key question:
- Which chunks produce later application, not just short-term recall?

### 18.3 Recovery capacity
Measure whether a learner can bounce back after failure.

Recommended fields:
- `first_failure_recovery_rate`
- `second_failure_recovery_rate`
- `continue_after_recovery_rate`
- `later_correct_use_after_recovery`
- `recovery_time_ms`

Key question:
- Which repair and support patterns produce durable recovery rather than temporary compliance?

### 18.4 Pace mismatch
Measure mismatch between the learner's preferred/usable pace and the runtime's pace.

Recommended fields:
- `reveal_speed_up_count`
- `replay_count`
- `waiting_on_tutor_proxy`
- `waiting_on_learner_proxy`
- `skipped_reveal_rate`
- `high_accuracy_fast_progress_rate`
- `slow_progress_stable_accuracy_rate`

Key question:
- Is the runtime too slow for strong learners or too dense for weaker learners?

### 18.5 Learning efficiency
Measure learning gain relative to time and turn cost.

Recommended fields:
- `learning_gain_per_minute`
- `retention_gain_per_minute`
- `turns_to_first_success`
- `turns_to_stable_success`
- `chunk_efficiency_score`

Key question:
- Which runtime pattern produces the most durable learning per unit of learner effort and time?

### 18.6 AI-change quality
If AI is proposing or implementing app changes, measure the quality of those changes as a system.

Recommended fields:
- `ai_change_id`
- `ai_change_type`
- `ai_change_release_date`
- `metric_improved` jsonb
- `metric_worsened` jsonb
- `rollback_required` boolean
- `time_to_detect_bad_change_days`
- `prompt_change_win_rate`
- `ui_change_win_rate`
- `runtime_rule_change_win_rate`

Key question:
- Is the AI-led improvement loop producing net gains, or creating churn and regressions?

## 19. Additional Dashboards

Add these dashboards after the initial set is stable:

7. Support dependence dashboard
- hint dependence, repair dependence, unsupported success, support fade success

8. Transfer dashboard
- near-transfer and later-transfer outcomes by chunk and lesson type

9. Pace mismatch dashboard
- reveal usage, replay usage, speed-up usage, accuracy vs pace pattern

10. Learning efficiency dashboard
- learning gain per minute, turns to success, chunk efficiency by format

11. AI-change quality dashboard
- changes shipped, win/loss rate, rollback rate, time to detect regressions

## 20. Additional Success Criteria For Telemetry

The telemetry should eventually be able to support decisions like:
- reduce scaffold intensity because unsupported success is rising and hint dependence is falling
- add a transfer prompt to specific chunks because recall is strong but transfer is weak
- speed up reveal for stronger learners because pace mismatch is visible in skip and replay patterns
- remove a prompt change because it improved completion but worsened learning efficiency
- roll back an AI-led UI change because its regression was detected in retention and recovery metrics

## 21. Recommendation

After the core telemetry is live, prioritize these additions in this order:
