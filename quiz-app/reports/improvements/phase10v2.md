...
interface RubricScore {
  ...
  breakdown: {
    schemaCompliance: number;    // A: 15 points
    pedagogy: number;            // B: 30 points (beginner clarity)
    questions: number;           // C: 25 points (alignment + quality)
    marking: number;             // D: 10 points (robustness)
    visual: number;              // E: 5 points
    safety: number;              // F: 5 points (auto-caps)
  };
...
interface RubricDetail {
  section: string;              // e.g., "markingRobustness: ..."
  score: number;
  maxScore: number;
  issues: string[];
  suggestions: string[];
...
**Location:** `llmScoringService.ts` lines 217-400
...
**Location:** `llmScoringService.ts` lines 406-422
...
**Location:** `Phase10_Rewrite.ts` lines 207-243
...
- Emphasizes structural invariants (DO NOT change IDs, types, orders, block count)
...
**Location:** `Phase10_Rewrite.ts` lines 250-267
...
**Scoring Report Format** (lines 273-290):
...
2. Scoring report with total score and grade
