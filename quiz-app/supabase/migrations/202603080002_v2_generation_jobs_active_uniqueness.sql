...
set
  ...
  locked_by = null,
  locked_at = null,
  error_message = coalesce(error_message, 'Cancelled by migration: duplicate active lesson_draft job')
from ranked
where jobs.id = ranked.id
  and ranked.row_num > 1;

create unique index if not exists idx_v2_generation_jobs_one_active_lesson_draft
