          ...
          {content.imageUrl && (
            <div className="bg-gray-100 dark:bg-slate-800 rounded-lg p-4 border-2 border-gray-300 dark:border-slate-600">
              {isSvgImage ? (
                ...
                >
                  <img src={content.imageUrl} alt="Diagram to label" className="w-full h-auto rounded" />
                </object>
              ) : (
                <img src={content.imageUrl} alt="Diagram to label" className="w-full h-auto rounded" />
              )}
          ...
          {!showResults ? (
            <>
              ...
              <div className="microbreak-enter-slow microbreak-card-glide bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-200 dark:border-blue-700 rounded-lg p-4 text-center">
                <p className="text-lg font-semibold text-gray-900 dark:text-white">What is this component?</p>
                <p className="text-sm text-gray-600 dark:text-slate-400 mt-1">Position: {currentItem.id}</p>
              </div>
                ...
                {availableLabels.map((label, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 text-sm font-medium microbreak-stagger microbreak-card-glide ${
                        isUsed ? choiceButtonClass('used') : choiceButtonClass('idle')
                      }`}
                    >
                      {label}
                    </button>
            ...
            <div className="space-y-3">
              <h4 className="font-semibold text-gray-900 dark:text-white text-center">Results</h4>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-lg">{isCorrect ? 'OK' : 'NO'}</span>
                    </div>
                    {!isCorrect && (
                      <p className="text-xs text-red-700 dark:text-red-300 mt-1">You answered: {userAnswer}</p>
                    )}
                  </div>
                );
              })}
              <div className="text-center text-gray-700 dark:text-slate-300 font-semibold">
                {Math.round(
                  (content.items.filter((item) => userAnswers[item.id] === item.label).length /
