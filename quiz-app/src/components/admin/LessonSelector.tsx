  ...
  return (
    <div className="space-y-2">
      ...
      <select
        ...
        disabled={disabled}
        className="w-full px-4 py-3 text-base rounded-lg border-2 border-gray-300 dark:border-slate-600 bg-white/90 dark:bg-slate-800 text-gray-900 dark:text-white focus:border-cyan-500 dark:focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-500 dark:focus:ring-cyan-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
      >
          ...
          <optgroup key={unit} label={unit}>
            {unitLessons.map((lesson) => (
              <option key={lesson.filename} value={lesson.filename}>
                {lesson.microbreakCount > 0 ? '[Has Games]' : '[No Games]'} {lesson.id} - {lesson.title.substring(0, 60)}
