...
export default function GamePreview({ games }: GamePreviewProps) {
  ...
  const getGameTypeColor = (content: MicrobreakContent) => {
    ...
    switch (content.gameType) {
      ...
      case 'scenario-match': return 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 border-blue-300 dark:border-blue-700';
      case 'formula-build': return 'bg-fuchsia-100 dark:bg-fuchsia-900/30 text-fuchsia-800 dark:text-fuchsia-300 border-fuchsia-300 dark:border-fuchsia-700';
      case 'tap-the-line': return 'bg-pink-100 dark:bg-pink-900/30 text-pink-800 dark:text-pink-300 border-pink-300 dark:border-pink-700';
      case 'tap-the-word': return 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 border-red-300 dark:border-red-700';
  ...
  const renderGameContent = (content: MicrobreakContent) => {
    ...
    switch (content.gameType) {
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              {content.pairs.map((pair, idx) => (
                <div key={idx} className="rounded p-2 text-sm flex items-center gap-2 bg-cyan-50 dark:bg-cyan-900/20">
        ...
        return (
          <div className="space-y-2">
            <div className="mb-2 flex gap-2">
              <span className="rounded-full bg-emerald-200 dark:bg-emerald-800 px-3 py-1 text-sm font-medium text-emerald-900 dark:text-emerald-100">
                {content.buckets[0]}
              </span>
              <span className="rounded-full bg-emerald-200 dark:bg-emerald-800 px-3 py-1 text-sm font-medium text-emerald-900 dark:text-emerald-100">
                {content.buckets[1]}
            ...
            <div className="space-y-1">
              {content.items.map((item, idx) => (
                <div key={idx} className="text-sm rounded p-2 flex justify-between bg-emerald-50 dark:bg-emerald-900/20">
      ...
      case 'spot-error':
        return (
          <div className="space-y-2">
            <p className="rounded p-2 text-sm text-gray-700 dark:text-slate-300 bg-amber-50 dark:bg-amber-900/20">
            ...
            <div className="space-y-1">
              {content.options.map((option, idx) => (
                ...
                >
                  ...
                  {option.isError && <span className="ml-2 text-xs font-semibold text-red-600 dark:text-red-400">&lt;- ERROR</span>}
                </div>
              ))}
            </div>
            {content.explanation && (
              <p className="mt-2 text-sm italic text-gray-600 dark:text-slate-400">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              {content.questions.map((q, idx) => (
                <div key={idx} className="rounded p-2 text-sm bg-rose-50 dark:bg-rose-900/20">
                  <div className="font-medium text-gray-900 dark:text-white">Q: {q.question}</div>
                  <div className="ml-4 text-gray-700 dark:text-slate-300">A: {q.answer}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        return (
          <div className="space-y-1 text-sm">
            ...
            {content.steps.map((step, idx) => (
              <div key={`${step}-${idx}`} className="rounded bg-sky-50 dark:bg-sky-900/20 p-2">{step}</div>
            ))}
          </div>
        );
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-lg font-black tracking-tight text-gray-900 dark:text-white">Generated Games Preview</h3>
      ...
      {games.map((game) => {
        ...
        return (
          <div
            key={game.id}
            className="overflow-hidden rounded-xl border-2 border-gray-200 bg-white/80 dark:border-slate-700 dark:bg-slate-800/80"
          >
            <button
              onClick={() => toggleGame(game.id)}
              className="flex w-full items-center justify-between px-4 py-3 transition-colors hover:bg-gray-50/80 dark:hover:bg-slate-700/40"
            >
              <div className="flex items-center gap-3 text-left">
                <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${getGameTypeColor(game.content)}`}>
                  {getGameTypeLabel(game.content)}
                </span>
                <span className="text-sm font-medium text-gray-900 dark:text-white break-all">{game.id}</span>
